...
# Revisions
#  10/30/2015
#    - WA-PERS retirees had:
#        (a) some negative benefits
#        (b) some NA benefits where # employees>0
#        (c) some NA benefits where # employees is NA
#       I fixed this
#  10/7/2015
...
setwd("./Imputation_FromDon")
dir <- "./data-raw/"
fn <- "Prototype inputs(15).xlsx"
...
fillin.retirees <- function(lretirees) {
  ...
  avgben <- avgben %>% mutate(benefit=ifelse(benefit<0, 0, benefit))
...
plan <- "LA-CERA-43.fillin.yos" # AZ-PERS-6.fillin.yosLA-CERA-43.fillin.yos OH-PERS-85.fillin.yos WA-PERS2-119.fillin.yos
tmp <- actives %>% filter(planname==plan)
sum(tmp$nactives)
# tmp %>% mutate(yos=age-ea) %>% filter(yos<=4) %>% summarise(nactives=sum(nactives))
tmp %>% mutate(yos=age-ea) %>% filter(yos<=4) %>% group_by(ea) %>%
  summarise(nactives=sum(nactives)) %>% ungroup %>%
  mutate(agepct=nactives / sum(nactives)) %>%
  qplot(ea, agepct, data=., geom=c("point", "line"))


plan <- "OH-PERS-85.fillin.yos" # AZ-PERS-6.fillin.yos LA-CERA-43.fillin.yos OH-PERS-85.fillin.yos WA-PERS2-119.fillin.yos
tmp <- actives %>% filter(planname==plan)
tmp %>% mutate(yos=age-ea) %>% filter(yos<=1) %>% group_by(ea) %>%
  summarise(nactives=sum(nactives)) %>% ungroup %>%
  mutate(agepct=nactives / sum(nactives)) %>%
  summarise(avgage.ne=sum(ea * agepct))
#                        calcdata  calcspreadsheet lowest yos grp
# AZ-PERS-6.fillin.yos     38.7   39.3
# LA-CERA-43.fillin.yos    34.7   34.4
# OH-PERS-85.fillin.yos    33.6   34.7
# WA-PERS2-119.fillin.yos  37.7   37.6
