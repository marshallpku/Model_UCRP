...
#************************************************************************************************************************
#    ##  Check that the liability upon retirement can be produced from the mortality table with "dynamic weight"    ####
#************************************************************************************************************************
...
# This section will numercially verify that the uni-sex mortality based on dynamic weights can generate interally consistent results. 
...
df.w <- expand.grid(age = age.range, age.r = age.r.range) %>% 
               ...
               mutate(
                 ...
                 pRxm.M     = male.pct   * ifelse(age == min(age), 1, lag(cumprod(pxm.post.M))),
                 pRxm.F     = female.pct * ifelse(age == min(age), 1, lag(cumprod(pxm.post.F))),
                 ...
                 COLA.scale = (1 + COLA)^(row_number() - 1 ),
                 B =  COLA.scale,
                 ax.r.R.W     =  get_tla(pxm.post.W, i, COLA.scale),
                 liab.la.W = B * ax.r.R.W    # "la" for life annuity
               )


df.all %>% filter(age == age.r) %>% select(age, liab.tot) %>% 
           left_join(df.w %>% filter(age == age.r) %>% select(age, liab.la.W)) %>% 
           mutate(liab.diff = liab.tot - liab.la.W,
                  equal     = liab.tot == liab.la.W)

# differences are neglectable.
# Conclusion: liability of the life annuity for actives should be calculated with the dynamically weighted mortality table.



#************************************************************************************************************************
#    ##  Check the difference in cashflow between life annuity and contingent annuity    ####
#************************************************************************************************************************

df.la_ca <- df1.M %>% select(age.r, age, B.la = B.R, liab.tot.la) %>% 
            left_join(df2.M %>% select(age.r, age, B.ca = B.tot, liab.tot.ca = liab.tot)) %>% 
            mutate(liab.ca_la = 100 * liab.tot.ca / liab.tot.la,
                   B.ca_la    = 100 * B.ca / B.la) 


df.la_ca %>% filter(age.r %in% seq(55,70, 5), age %in% seq(50, 120, 5))

df.la_ca %>% filter(age.r %in% 53, age %in% seq(50, 120, 5))
df.la_ca %>% filter(age.r %in% 60, age %in% seq(50, 120, 5))
df.la_ca %>% filter(age.r %in% 70, age %in% seq(50, 120, 5))

# The differences are relatively small before age 80, and rise rapidly after 80.
